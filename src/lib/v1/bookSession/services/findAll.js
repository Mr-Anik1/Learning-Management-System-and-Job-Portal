...
const findAll = async ({
  ...
  try {
    ...
    // Find all bookSessions
    const bookSessions = await BookSession.find(filter)
      .sort(sortStr)
    ...
    // If the array of bookSessions doesn't exist or is empty
    if (!bookSessions || bookSessions.length === 0) {
      throw new errors.NotFoundError(`Your requested resource doesn't exist`);
    }

    // Return the array of bookSessions
    return bookSessions;
  } catch (err) {
    if (err.message) {
      console.log(`[FIND_ALL_BOOK-SESSIONS]: ${err.message}`);
    }
