...
const findAll = async ({
  ...
  try {
    ...
    // Find all project category
    const projectCategories = await ProjectCategory.find(filter)
      .sort(sortStr)
      .skip(page * limit - limit)
      .limit(limit);

    // If the array of projectCategories doesn't exist or is empty
    if (!projectCategories || projectCategories.length === 0) {
      throw new errors.NotFoundError(`Your requested resource doesn't exist`);
    }

    // Return the array of projectCategories
    return projectCategories;
  } catch (err) {
    if (err.message) {
      console.log(`[FIND_ALL_PROJECT_CATEGORIES]: ${err.message}`);
    }
